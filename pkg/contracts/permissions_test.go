...
func TestCoreWireConstants(t *testing.T) {
	...
	if ScopeServiceRegister != "service.register" || ScopeServiceConfigRead != "service.config.read" || ScopeServiceSecretResolve != "service.secret.resolve" || ScopeStreamsStart != "streams.start" || ScopeStreamsStop != "streams.stop" || ScopeObservabilityIngest != "observability.ingest" || ScopeNotificationsEmailSend != "notifications.email.send" || ScopeRemediationExecute != "remediation.execute" || ScopeUpdatesClaim != "updates.claim" || ScopeUpdatesReport != "updates.report" || ScopeUpdatesAuthorize != "updates.authorize" {
